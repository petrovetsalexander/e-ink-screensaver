...
- **Hardware frontlight — does not do what it was built for.** `setScreenBrightnessLevel(int)` is nothing but `SystemProperties.set("vendor.xrz.global_brightness_level", …)`. Worse, an on-device run showed that by the time `ACTION_SCREEN_OFF` reaches `onScreenOff()` the system has *already* zeroed that property, so `dimFrontlight()` hits its `current <= 0` guard and does nothing. Dimming at screen-off is redundant; whatever lights the panel does so later, when the activity turns the screen back on. `dimFrontlight()` / `restoreFrontlight()` still save the user's level in prefs (`saved_frontlight_level`, sentinel `PrefsManager.NO_SAVED_FRONTLIGHT`) so a process death can't strand the light at 0; the service restores it in `onCreate`. Restore paths: the unlock poll in `cancelNotificationAndFinish()` (the reliable one), `USER_PRESENT`, `ACTION_STOP`, `onDestroy`.
