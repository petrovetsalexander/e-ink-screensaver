...
- **Hardware frontlight — unproven.** `setScreenBrightnessLevel(int)` turns out to be nothing but `SystemProperties.set("vendor.xrz.global_brightness_level", …)`, and that property is `vendor_xrz_prop` on a device running SELinux Enforcing, so a third-party app may well be refused; `setFrontlight` reads the value back and reports whether the write took. The level also sits at 0 already on this device, which means `dimFrontlight()` short-circuits and the frontlight flash on lock probably comes from Android's own brightness path (`settings system screen_brightness`), not from the xrz level. `dimFrontlight()` / `restoreFrontlight()` drive the panel light directly, so `ScreenSaverService.onScreenOff()` can kill it *before* any window of ours exists — `window.screenBrightness = 0.0f` only applies once `LockScreenActivity`'s window is added, which is too late to stop the system's dim level from flashing the light. The user's level is saved in prefs (`saved_frontlight_level`, sentinel `PrefsManager.NO_SAVED_FRONTLIGHT`) so a process death can't strand the light at 0; the service restores it in `onCreate`. Restore paths: the unlock poll in `cancelNotificationAndFinish()` (the reliable one), `USER_PRESENT`, `ACTION_STOP`, `onDestroy`.
