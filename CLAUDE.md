...
**With the vendor framework present but the tagged wake unavailable, the app now draws nothing at all.** `onScreenOff` and `onClockAlarmFired` both bail out (`LOCK_SKIP` / `TICK_SKIP`) when `EinkCompat.isSupported && !canUseVendorWake()`. Measured on the HiBreak: the plain wake brings the panel up at full frontlight, nothing can put the device back to sleep, so it burns until `screen_off_timeout` (120 s here) — and while the device is interactive with our window over the keyguard, the keyguard stops listening to the fingerprint sensor, so the phone cannot be unlocked by touch either. A dashboard is not worth either of those; the phone stays asleep and usable and the notification says why. Hardware without the xrz framework is unaffected — there the plain wake *is* the design.

A force-stop is what usually kills the binding, and `debug_app` is a spectacular way to get one: with the package selected in Developer options → *Select debug app*, every re-apply of that setting force-stops it (`ActivityManager: Force stopping … : set debug app`), after which `dumpsys accessibility` lists the service under `Crashed services:` and never rebinds. Check `settings get global debug_app` before blaming anything else.
...
- `settings/SettingsActivity` — service on/off, permission gates, language, entry to modules, about/how-it-works dialogs, and last on the screen the button into `DiagnosticsActivity`. The permission block is one row per permission — label with ✓/✗, a `?` that opens `showPermissionHelp` with that permission's hint string, and its grant button — plus `a11yLostWarning`, the one message shown inline because it reports a change rather than explaining a choice. Adding a permission means a row in `activity_settings.xml`, a title/hint string pair in **both** `values/` and `values-ru/`, and its line in `updateUI`.
...
- `settings/DiagnosticsActivity` — the event-log switch, share and clear, and `EventLog.snapshot` shown live in a monospace block, which answers "is the accessibility service actually bound" without anyone having to send a file first. Off the bottom of the settings screen and out of the way: chasing a bug is not part of setting the app up.
